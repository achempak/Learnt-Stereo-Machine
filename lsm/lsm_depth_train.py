...
from lsm_depth import LSM_depth
...
categories = ['sofa']
#categories = None
dataset = ShapeNetDataset(im_dir, vox_dir, nviews, nvox, split_file, train=True, categories=categories, include_depth=True)
train_batch_loader = torch.utils.data.DataLoader(dataset, batch_size=batch_size)
test_dataset = ShapeNetDataset(im_dir, vox_dir, nviews, nvox, split_file, train=False, categories=categories, include_depth=True)
test_batch_loader = torch.utils.data.DataLoader(test_dataset, batch_size=batch_size)

### CHECKPOINTS
if not os.path.isdir('./checkpoints'):
    os.mkdir('./checkpoints')
checkpoint_path = './checkpoints/depth_{}'.format(str(int(time.time())))
...
def train(lsm, epochs, lr, train_batch_loader, test_batch_loader):
    ...
    for epoch in range(epochs):
        ...
        for i, batch in enumerate(train_batch_loader):
            ...
            optimizer.zero_grad()
            depth_pred = lsm(imgs)
            loss = loss_func(depth_pred, depth)
            loss.backward()
            optimizer.step()
