...
class ImUnet(nn.Module):
    def __init__(self):
        ...
        self.conv_1 = nn.Conv2d(in_channels=3, out_channels=32, kernel_size=5, stride=2, padding=2)
    ...
    def forward(self, image):
        ...
        x_deconv_1 = torch.cat((x_deconv_1, x_conv_3), dim=2)
        x_deconv_2 = self.deconv_2(F.relu(x_deconv_1))
        x_deconv_2 = torch.cat((x_deconv_2, x_conv_2), dim=2)
        x_deconv_3 = self.deconv_3(F.relu(x_deconv_2))
        x_deconv_3 = torch.cat((x_deconv_3, x_conv_1), dim=2)
